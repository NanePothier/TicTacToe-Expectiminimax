...
public class StateTree {
	...
	public StateNode[] stateTree = new StateNode[600]; //3 layers -> root + two layers
	int winState, favState, lowState;
	...
	public void generateTree(StateNode parent, int rootIndicator) {
		...
		for(int i = 0; i < currentState.length; i++){
			for(int j = 0; j < currentState.length; j++){
				...
				if(currentState[i][j] == 0){
					...
					//if leaf node then calculate heuristic using heuristic function
		...
		for(int x = 0; x < layerChildren.length; x++){
			...
			//if rootIndicator == 1, go ahead and compute 3rd layer children
			if(rootIndicator == 0){
	...
	//depending on value of branchCounter uneven number of children will be split differently
	public void generateBranchCounter() {
		branchCounter = random.nextInt(2);
	}
	...
	// split children into two branches and assign chance to each branch
	public void calculateChances(StateNode[]children, int numChildren) {
		...
		if(numChildren != 0){
			...
			if(firstHalf == 0){
				...
				chanceValues[1] = (tempValue - 0.05);
				chanceValues[2] = (tempValue + 0.05);
				chanceValues[3] = (tempValue - 0.04);
				chanceValues[4] = (tempValue + 0.04);	
			}
	...
	//calculate actual heuristic value for each branch (multiply chance value with each heuristic value of the same branch, add them up and add to heuristic value of children
	...
	public double expectiminimax(StateNode parent, int depth, boolean isMaximizingPlayer){
		...
		if(isMaximizingPlayer){
			...
			for(int x = 0; x < numChildren; x++){		
				value = expectiminimax(stateTree[childIndex], depth + 1, false);			
			...
			//calculate new heuristic values that include branch chance values
